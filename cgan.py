...
def train(dataset, epochs,dataset_size,var):
  ...
  for epoch in range(epochs):
    ...
    if (abs(epoch_r_acc-50) + abs(epoch_f_acc-50) <= best) and epoch > 200: 
        ...
        discriminator.save(MODEL_FOLDER + '{}_gan_discriminator_{}.h5'.format(var if not var is None else 'all',epoch+1))
...
if __name__ == "__main__":
    ...
    train_dataset = tf.data.Dataset.from_tensor_slices((train_data,real_values)).shuffle(train_data.shape[0]).batch(BATCH_SIZE)
